...
namespace ProductSale.Lib.Infra.Repo
{
	public class CategoryRepository : ICategoryRepository
    {
        ...
        public async Task<int> UpsertCateogryAsync(Category category) {
            ...
            if (category.Id == 0)
            {
                return await queryFactory.StatementAsync("exec InsertCategories @Name,@UserId", new {Name = category.Name, UserId = category.CreatedBy });
            }
        ...
        public async Task<List<Category>> GetAllCategoryAsync() {
            ...
            try
            {
                ...
                var results = queryFactory.Query(TableName)
                    .LeftJoin(SubCategory, join => join.On($"{SubCategory}.CatId",$"{TableName}.Id").Where($"{SubCategory}.IsActive",1))
